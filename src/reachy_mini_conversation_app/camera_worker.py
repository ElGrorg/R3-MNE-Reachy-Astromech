...
class CameraWorker:
    ...
    def _trigger_face_detection_event(self) -> None:
        """Trigger a sound/action when a face is detected."""
        try:
            # Play a random chirp/happy sound
            # We need to find the audio directory. 
            # This file is in src/reachy_mini_conversation_app/camera_worker.py
            # Audio is in emotion_audio/ (root)
            ...
            # Use 'happy_short' or 'misc' for greeting
            target_dirs = ["happy_short", "misc"]
            target_dir_name = random.choice(target_dirs)
            target_dir = os.path.join(audio_root, target_dir_name)
            ...
            if os.path.exists(target_dir):
                ...
                if files:
                    ...
                    logger.info(f"Playing face detection sound: {sound_path}")
                    player = SoundPlayer(sound_path)
                    player.play()
        except Exception as e:
            logger.error(f"Error triggering face detection event: {e}")
                    ...
                    if self.is_head_tracking_enabled and self.head_tracker is not None:
                        ...
                        if eye_center is not None:
                            ...
                            if self.last_face_detected_time is None:
                                # First time detecting face (or after lost)
                                logger.info("Face detected! Triggering greeting.")
                                self._trigger_face_detection_event()
