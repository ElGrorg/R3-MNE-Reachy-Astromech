...
class Dance(Tool):
    ...
    async def __call__(self, deps: ToolDependencies, **kwargs: Any) -> Dict[str, Any]:
        ...
        # Play sound (happy/energetic for dance)
        try:
            import os
            import random
            ...
            # Pick a happy sound folder
            candidates = ["happy_short", "happy_long"]
            chosen_folder = random.choice(candidates)
            target_dir = os.path.join(audio_root, chosen_folder)
            ...
            if os.path.exists(target_dir):
                ...
                if files:
                    ...
                    logger.info(f"Playing dance sound: {sound_path}")
                    player = SoundPlayer(sound_path, head_wobbler=deps.head_wobbler)
                    player.play()
            else:
                logger.warning(f"Could not find audio directory for dance: {target_dir}")

        except Exception as e:
            logger.error(f"Error playing dance sound: {e}")
